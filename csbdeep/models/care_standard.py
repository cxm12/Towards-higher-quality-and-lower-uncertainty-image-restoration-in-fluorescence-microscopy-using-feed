...
class CARE(BaseModel):
    ...
    def __init__(self, config, name=None, basedir='.', scale=1):
        """See class docstring."""
        self.scale = scale
    ...
    def _build(self):
        if self.scale == 1:
            ...
            return nets.common_unet(
                ...
                kern_size=self.config.unet_kern_size,
                n_first=self.config.unet_n_first,
                last_activation=self.config.unet_last_activation, scale=self.scale
            )(self.config.unet_input_shape)
    ...
    def train(self, X,Y, validation_data, epochs=None, steps_per_epoch=None):
        ...
        self.keras_model.save_weights(str(self.logdir) + '/weights_ep%d.h5' % (epochs))
    ...
    def Valid_on_testsetdenoise(self):
    ...
    def Valid_on_testsetSR(self, validation_data):
        ...
        for i in range(imnum):
            ...
            y = y[:, (H // 4):(H - H // 4), (W // 4):(W - W // 4)]
            restoredraw = self.predict(x, axes, n_tiles=(1, 4, 4))
            restored = normalize(restoredraw, 0.1, 99.9) * 255

            try:
                savecolorim(spath + 'im%d-dZ%d' % (i, z) + '.tif', np.uint8(restored[z, :, :]))
                dif = np.abs(normalize(y[z, :, :], 0.1, 99.9) * 255 - restored[z, :, :])
                imsave(spath + 'im%d-dZ%d' % (i, z) + 'Dif.tif', dif)
            except:
                savecolorim(spath + 'im%d-dZ%d' % (i, z) + '.png', np.uint8(restored[z, :, :]))
                dif = np.abs(normalize(y[z, :, :], 0.1, 99.9) * 255 - restored[z, :, :])
                misc.imsave(spath + 'im%d-dZ%d' % (i, z) + 'Dif.png', dif)
        
            restored = np.swapaxes(restored, 0, 2)
            y = np.swapaxes(y, 0, 2)
            psm, ssmm = compute_psnr_and_ssim(restored, normalize(y, 0.1, 99.9) * 255)
            # print('Image - ', name, '- PSNR/SSIM = ', ps, ssm, '- PSNR/SSIM of mean test = ', psm, ssmm)
            meanpsnr += psm
            meanssim += ssmm
        print('meanssim, meanpsnr', meanssim / imnum, meanpsnr / imnum)
