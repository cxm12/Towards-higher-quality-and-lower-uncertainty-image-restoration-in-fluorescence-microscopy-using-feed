...
callbacks = keras_import('callbacks')
    ...
    def set_model(self, model):
            ...
            for layer in self.model.layers:
                for weight in layer.weights:
                    if IS_TF_1:
                        ...
                        tf_sums.append(tf1.summary.histogram(weight.name, weight))

                if hasattr(layer, 'output'):
                    if IS_TF_1:
                        tf_sums.append(tf.summary.histogram('{}_out'.format(layer.name), layer.output))
                    else:
                        tf_sums.append(tf1.summary.histogram('{}_out'.format(layer.name), layer.output))
        ...
        for i,sl in zip(image_for_inputs,input_slices):
            ...
            if IS_TF_1:
                ...
                tf_sums.append(tf1.summary.image(layer_name, input_layer, max_outputs=self.n_images))
        ...
        for i, sl in zip(image_for_outputs,output_slices):
            ...
            if IS_TF_1:
                ...
                tf_sums.append(tf1.summary.image(layer_name, output_layer, max_outputs=self.n_images))
            # prediction
            ...
            if self.prob_out:
                ...
                if IS_TF_1:
                    ...
                    tf_sums.append(tf1.summary.image(mean_name, output_layer, max_outputs=self.n_images))
                    tf_sums.append(tf1.summary.image(scale_name, scale_layer, max_outputs=self.n_images))
            else:
        ...
        with tf.name_scope('merged'):
            if IS_TF_1:
                ...
                self.merged = tf1.summary.merge(tf_sums)
        ...
        with tf.name_scope('summary_writer'):
            if self.write_graph:
                if IS_TF_1:
                    ...
                    self.writer = tf1.summary.FileWriter(self.log_dir, self.sess.graph)
            else:
                if IS_TF_1:
                    self.writer = tf.summary.FileWriter(self.log_dir)
                else:
                    self.writer = tf1.summary.FileWriter(self.log_dir)
...
class CARETensorBoardImage(Callback):
    ...
    def __init__(self, model, data, log_dir,
        ...
        if IS_TF_1:
            ...
            self.file_writer = tf1.summary.create_file_writer(str(self.log_dir))
            ...
            with self.file_writer.as_default():
                for i, sl in zip(self.image_for_inputs, self.input_slices):
                    ...
                    if IS_TF_1:
                        ...
                        tf1.summary.image(input_name, input_image, step=epoch)
            ...
            for i,sl in zip(self.image_for_outputs,self.output_slices):
                ...
                if self.prob_out:
                    ...
                    if IS_TF_1:
                        ...
                        tf1.summary.image(output_name, output_image, step=epoch)
                        tf1.summary.image(scale_name, scale_image, step=epoch)
            ...
            with self.file_writer.as_default():
                for i, sl in zip(self.image_for_outputs, self.output_slices):
                    ...
                    if IS_TF_1:
                        ...
                        tf1.summary.image(target_name, target_image, step=epoch)
